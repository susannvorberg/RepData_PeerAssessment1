...
ggplot(plot_df, aes(x=interval, y=values, group=ind)) + 
    geom_point() +
    geom_line() +
    facet_wrap(~ ind) +  
    ggtitle("Average number of steps per interval for all dates") + 
...
We can see that the maximum number of steps is higher on weekdays than on weekends. However, on weekends the amount of intervals with intermediate step numbers (say, 100-200) is higher compared to weekdays.
